    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-50 via-white to-violet-50/40">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 py-16">
        ...
        <p className="mx-auto mt-5 max-w-xl text-center text-base leading-relaxed text-slate-600">
          Initialize your brain from a GitHub repo (and optional files). You will get the live session builder—clone, scan, distill, and graph—then open the full command center when you are ready.
        </p>
        ...
        <div className="mt-10 flex w-full max-w-md flex-col gap-3 sm:max-w-lg sm:flex-row sm:justify-center">
          ...
          >
            <Brain size={17} className={hasBrain ? "text-violet-600" : "text-slate-400"} />
            Open brain map
          </Link>
        </div>
        ...
        {!hasBrain ? (
          <p className="mt-4 max-w-md text-center text-xs text-slate-500">
            No brain files on disk yet—use <span className="font-semibold text-slate-700">Start new session</span> first, or open the map to see an empty workspace.
          </p>
        ) : (
          <p className="mt-4 max-w-md text-center text-xs text-slate-500">
            You already have brain content. Open the map to browse and chat, or start another session to re-initialize (overwrites when you choose that in the builder).
          </p>
        )}
      </div>
    </div>
